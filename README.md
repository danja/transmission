...
The current editor is a deliberately small native canvas: it renders node/arc topology, identifies system audio input/output nodes with correctly directed sockets, opens per-channel external-connection dialogs for those nodes, discovers JACK ports when enabled, applies JACK connections to the running engine, provides a tempo/play-stop/reset/loop transport bar backed by `TransportClock`, discovers bundles from `~/.vst3`, opens a modal plugin chooser from the graph background’s context menu, creates plugin nodes carrying their bundle paths, supports dragging nodes, and allows new audio arcs to be drawn from output sockets to input sockets. Project loading, graph editing commands, and native/Node synchronization are the next UI slices.
