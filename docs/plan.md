...
9. Build graph editing and plugin browsing. *(native GTK graph canvas prototype complete: correctly directed system I/O sockets, per-channel external-connection dialogs, on-demand JACK port discovery and connect/disconnect calls, transport controls backed by `TransportClock`, node dragging, output-to-input arc creation, modal right-click `~/.vst3` bundle browsing with plugin-node creation, and double-click native VST3 editor embedding; project synchronization, editing commands, and plugin browser metadata remain)*
