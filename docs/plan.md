...
9. Build graph editing and plugin browsing. *(native GTK graph canvas prototype complete: typed audio/MIDI ports and colored routes, correctly directed system I/O sockets, per-channel external-connection dialogs, on-demand JACK port discovery and connect/disconnect calls, transport controls backed by `TransportClock`, node context menus for edit/remove, node dragging, type-compatible output-to-input arc creation, modal right-click `~/.vst3` bundle browsing with plugin-node creation, and double-click native VST3 editor embedding; project synchronization, editing commands, MIDI device mapping, and plugin browser metadata remain)*
